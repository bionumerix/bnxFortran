...
CODE <- function(path, pattern) {
  annos <- "(^\\s*$)|(^\\s*#)|(^\\s*//)|(^\\s*/\\*)|(^\\s*\\*)|^\\s*C"
  files <- list.files(path, pattern, recursive = TRUE, ignore.case = TRUE,
                      full.names = TRUE)
  lines <- unlist(lapply(files, readLines))
  grep(annos, lines, value = TRUE, invert = TRUE)
}
size <- c(
  "R"       = length(CODE("R", pattern = "\\.R$")),
  "Tests"   = length(CODE("tests", pattern = "\\.R$")),
  "C"       = length(CODE(c("src", "inst"), pattern = "\\.h*$|\\.c*$")),
  "Fortran" = length(CODE("src", pattern = "\\.f*$"))
)
```
...
cat("**Lines of code:** ", paste(names(size), size, sep = " = ",
                                 collapse = "; "))
```
