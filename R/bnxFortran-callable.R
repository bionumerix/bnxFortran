...
##'
##' All callable functions can be called directly from other package source
##' code. For details see the details of respective functions listed below.
##' Function argument validation, except otherwise stated, shall usually be done
##' by the caller.
##'
...
##' @param x An double vector from e.g. \code{as.double()}. A numeric vector
##'   with the x-values (abscissas) of input data points in ascending order.
##' @param y An double vector from e.g. \code{as.double()}. A numeric vector
##'   with the y-values (ordinates) of input data points.
##' @param rn An integer scalar from e.g. \code{as.integer()}. The number of
##'   points at which interpolation of the y-value (ordinate) is desired. Must
##'   be 1 or greater.
...
##'   follows: \cr \emph{int(*BF_C_intrpl)(int *n, double *x, double *y, int
##'   *rn, double *rx, double *ry, int *err);} \cr\cr Within the
##'   \code{R_init_...} function, the callable function is imported via \cr
##'   \emph{BF_C_intrpl = (int(*)(int*, double*, double*, int*, double*,
##'   double*, int*)) R_GetCCallable("bnxFortran", "BF_C_intrpl");}.
NULL
...
##' @section Callable: The callable function is defined in a C header file as
##'   follows: \cr \emph{int(*BF_C_uvip3p)(int *np, int *n, double *x, double
##'   *y, int *rn, double *rx, double *ry, int *err);} \cr\cr Within the
##'   \code{R_init_...} function, the callable function is imported via \cr
...
##'   vector. On entry contains the \code{N} vector. On exit contains
##'   \code{Q*B}.
...
##'   double *B, double *X, double *RNORM, double *W, double *ZZ, int *INDEX,
##'   int *MODE, int *NSETP);} \cr\cr Within the \code{R_init_...} function, the
